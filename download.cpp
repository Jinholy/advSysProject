...
extern "C" bool download(const char* url, const char* ext){              
    ...
    if(usr_cmd == "y"){
        for(int i=0; i< cnt; i++){
            string c_url = url;                 
            string tmp = "wget -q -o /dev/null -b "+ c_url +file_list[i];
            //wget + 실행할 때 파라메터로 넘겨준 url주소 + extList에서 읽어들인 파일이름 -b 명령어로 background 에서 실행
